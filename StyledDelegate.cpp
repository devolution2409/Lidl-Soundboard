...
StyledDelegate::StyledDelegate(LIDL::HoverBehavior behavior, QObject *parent) : QStyledItemDelegate(parent)
{
        ...
        else
        {
            if (hoverBehavior != LIDL::HoverBehavior::HoverItems)
            {
                ...
                for (int i = 0; i < count; ++i)
                {
                    ...
                    if (i == (hoverBehavior == LIDL::HoverBehavior::HoverRows ? index.column() : index.row()))
                        continue;
